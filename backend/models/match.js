...
class Match {
  ...
  static findMatchList = async (user_id) => {
    ...
    const sql = "SELECT profile.user_id, profile.bio, profile.firstname, profile.birthday FROM profile LEFT JOIN liked ON liked.user_id = profile.user_id WHERE liked.user_id IS NULL ";
